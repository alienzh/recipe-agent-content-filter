...
async function setupServer(recreate = false, quiet = false, upgradePip = true) {
  ...
  install.push('-r', path.join(server, 'requirements.txt'))
...
async function runBackend(withSetup: boolean) {
  if (withSetup) await setupServer(false, true, false)
  else requireVenv()
...
function doctor(local = false) {
  ...
  for (const key of ['AGORA_APP_ID', 'AGORA_APP_CERTIFICATE', 'CUSTOM_LLM_URL']) {
    const value = file.match(new RegExp(`^${key}=(.+)$`, 'm'))?.[1]
    if (!value?.trim()) throw new Error(`${key} is missing in server/.env.local`)
    console.log(`- ${key} configured`)
  }
  if (/^CUSTOM_LLM_URL=.*(localhost|127[.]0[.]0[.]1)/im.test(file)) {
    console.warn('- WARNING: CUSTOM_LLM_URL points at localhost; use a public tunnel URL.')
  }
}
...
async function verifyBackend() {
  requireVenv()
  await run([venvPython, '-m', 'py_compile', 'server.py', 'agent.py', 'llm.py'], path.join(server, 'src'))
}
...
try {
  switch (command) {
    ...
    case 'setup-done': setupDone(); break
